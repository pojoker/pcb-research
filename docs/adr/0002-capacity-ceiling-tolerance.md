...
2026-08-28，用户通过 H4 接受选项 C 的规则形状：**无全局数值默认，按精确 scope 校准；证据或样本不足时一律 `indeterminate`；批复产能只作弱上界。** 本决定没有批准任何真实项目容差值，当前获批容差实例数为 0。机器可读政策见 `data/canonical/tolerance_policy.json`。

DP6 的职责是把推断产量与同口径的可比产能天顶作单边比较；容差不能用来修补主体、厂址、产品族、FAB、期间、指标、单位、币种或合并口径不一致。
...
H4 已采用 C 作为规则形状；没有校准样本时不得填入任意数字。每个后续数值实例仍必须把适用的精确 scope 写入 `tolerance-adr.schema.json`，不得写成全局默认。
...
- ADR 规则已经获批，但没有精确 scope 的数值实例时，真实推断行仍必须因 `DP6_TOLERANCE_NOT_APPROVED_OR_MISSING` 保持 `indeterminate`，不能发布。
...
2. 对该 C 类实例，`τ_round`、`τ_cap`、Q 分位数、校准窗口和最小样本数分别是多少？
3. 该实例采用哪一份产销量表或建成产能自述作为强天顶？批复产能继续只作弱上界。

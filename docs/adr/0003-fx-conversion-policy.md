...
2026-08-28，用户通过 H4 接受以下政策：**目标币种为 CNY，原币强制保留；存量使用期末官方收盘价，非报价日取期末前最后一个官方报价；流量优先使用交易日汇率；期间平均必须另过波动人闸。** 本决定没有批准任何实际汇率表、波动阈值或折算实例，当前实例化换算数为 0。机器可读政策见 `data/canonical/fx_policy.json`。

本 ADR 用于在保留原币事实的前提下，给跨主体/跨地区的金额指标提供可复算的显示或比较值；它不改变公司原始披露，也不把物理数量转换成金额。
...
1. 是否要求同时提供 `USD` 视图？
2. 若只取得期间汇总，显著波动的具体判定阈值是什么（例如基于日度汇率相对平均值的最大偏离，需给出精确百分比/统计量）？
3. 期末前没有可用官方报价时是否允许向更早日期滚动，允许多久？禁止向后滚动。
